...
addQuestion = () => {
    const url = 'https://opentdb.com/api.php?amount=10&category=17&difficulty=easy';
    fetch(url)
        .then(data => data.json())
...
showQuestion = questions => {
    ...
    questions.forEach(question => {
        ...
        <button type="button" class="btn btn-danger">${falseNumber}</button>
        <button type="button" class="btn btn-success">${rightNumber}</button>
        </div>
        <div>
